...
def build_services(config: GmeowConfig) -> tuple[PgCache, CasAttachmentStore, PgSemanticIndex, None, SyncService]:
    ...
    semantic = PgSemanticIndex(postgres_dsn, config.embedding_model, config.embedding_endpoint, config.semantic_chunk_size, config.semantic_chunk_overlap)
    graph = None
    gmail = None
    service_account_info = config.service_account_info()
    user_credentials_info = config.user_credentials_info()
    if config.auth_mode == "service_account" and config.subject and (service_account_info is not None or config.service_account_file.exists()):
        gmail = GoogleGmailClient(config.service_account_file if service_account_info is None else None, config.subject, service_account_info=service_account_info)
    elif config.auth_mode == "user_oauth" and (user_credentials_info is not None or config.user_credentials_file.exists()):
        gmail = UserOAuthGmailClient(config.user_credentials_file if user_credentials_info is None else None, credentials_info=user_credentials_info)
...
def create_app(config: GmeowConfig | None = None) -> FastAPI:
    ...
    def messages(limit: int = 50, offset: int = 0) -> list[dict[str, Any]]:
    ...
    def attachment(sha1: str):
        try:
            if hasattr(attachments, "get_bytes"):
                metadata = attachments.read_metadata(sha1)
                return Response(
                    attachments.get_bytes(sha1),
                    media_type=metadata.get("media_type") or metadata.get("source", {}).get("gmail", {}).get("mime_type") or "application/octet-stream",
                )
    ...
    def graph_projection(limit: int = 25, prefix: str | None = None, visibility: str = "user", kind: str | None = None, namespace: str | None = None) -> dict[str, Any]:
    ...
    def graph_rank(limit: int = 25, kind: str | None = None, predicate: str | None = None, namespace: str | None = None, visibility: str = "user", include_noise: bool = False) -> list[dict[str, Any]]:
        return cache.graph_ranked_nodes(limit=limit, kind=kind, predicate=predicate, namespace=namespace, visibility=visibility, include_noise=include_noise)

    @app.get("/api/v1/graph/centrality")
    def graph_centrality(metric: str = "pagerank", limit: int = 25, kind: str | None = None, namespace: str | None = None, visibility: str = "user") -> list[dict[str, Any]]:
        try:
    ...
    def graph_related_nodes(node: str, limit: int = 25, max_weight: float | None = None, kind: str | None = None, namespace: str | None = None, visibility: str = "user") -> list[dict[str, Any]]:
        return cache.graph_related_nodes(node=node, limit=limit, max_weight=max_weight, kind=kind, namespace=namespace, visibility=visibility)

    @app.get("/api/v1/graph/bridges")
    def graph_bridges(limit: int = 25, kind: str | None = None, namespace: str | None = None, visibility: str = "user") -> list[dict[str, Any]]:
    ...
    def graph_message_nodes(message_id: str, visibility: str = "user", kind: str | None = None, namespace: str | None = None) -> dict[str, Any]:
    ...
    def graph_top(prefix: str | None = None, limit: int = 25, include_noise: bool = False, kind: str | None = None, namespace: str | None = None, visibility: str = "user") -> list[dict[str, Any]]:
        return cache.graph_top_nodes(prefix=prefix, limit=limit, include_noise=include_noise, kind=kind, namespace=namespace, visibility=visibility)
